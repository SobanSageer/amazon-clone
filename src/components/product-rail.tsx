...
export function ProductRail({ title, href, products }: {
  ...
  return (
    <section aria-labelledby={headingId} className="rounded-xl bg-white p-4 sm:p-5">
      <div className="mb-3 flex items-baseline justify-between gap-4">
        <h2 id={headingId} className="text-lg font-bold tracking-tight text-zinc-900 sm:text-xl">{title}</h2>
        {href && (
          <Link href={href} className="inline-flex shrink-0 items-center gap-0.5 rounded-sm text-sm font-medium text-sky-700 hover:text-sky-900 hover:underline">
